...
#Functions needed in the Boyce et al 2017 workflow to convert relative to absolute traveltime residuals
...
def GenerateWeightings(badfileslist,event,scheme='RMS',function=7,SNR_cutoff=3.5,XC_cutoff=0.90,XC_time_cuttoff=0.25):
...
def CheckErrors(badfileslist,event,XC_cutoff=0.90,XC_time_cuttoff=0.25):
	...
	for line in lines:
		...
		if (ttshift > XC_time_cuttoff) or (XCval < XC_cutoff):
			...
			print 'file will not be included in subsequent processing'
			badfileslist.write('%s %s\n' %(event,filename))
		else:
...
def AppendAbsoluteArrivals(datapath):

	'''Final stage: get the arrival time corrections from the stacked SAC files and append them to the alignment time (in the original files). Also add
	the correction from X correlation. Finally, compute the new traveltime residuals'''

	sacfiles = glob.glob('event_stacks/*.sac')
	events = []
	...
	evtcorrfile = open('STACK_TIME_CORRECTIONS.txt','w')
	...
	#now, open the eventcorr file, find the events for which we have corrections and create a new file for each event containing the informaton we need to use

	infile = open('STACK_TIME_CORRECTIONS.txt','r')
	lines = infile.readlines()
	infile.close()

	for line in lines:
		vals = line.split()
		event = vals[0].strip().split('/')[1]
		correction = float(vals[1])
		eventpath = datapath+'/'+event+'/BH_VEL'

		if os.path.exists(eventpath):

			print '--------------------------'
			print 'Event %s' %eventpath
			print '--------------------------'

			infile = open(eventpath+'/'+'FILES_TO_USE.txt','r')
			lines = infile.readlines()
			infile.close()
			outfile = open(eventpath+'/'+'Absolute_Arrivals.out','w')
			outfile.write('File    Predicted arrival     Relative arrival    XC correction    Absolute arrival    Relative residual    Absolute residial  XC value\n')
